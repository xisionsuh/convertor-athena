...
export default function MemoPanel({
  ...
  const MobileFloatingButton = () => (
    <button
      onClick={onToggle}
      className="md:hidden fixed bottom-20 right-4 z-50 bg-primary text-primary-foreground p-4 rounded-full shadow-lg hover:bg-primary/90 transition-all"
  ...
  // 데스크톱 닫힌 버튼
  const DesktopClosedButton = () => (
    <button
      onClick={onToggle}
      className="hidden md:block bg-background shadow-md p-3 rounded-l-xl hover:bg-muted z-10 text-foreground border border-r-0 border-border/50 transition-all hover:pr-4 group"
      title="Open Memo"
    >
  ...
  const MobileFullScreenPanel = () => (
    <div className="md:hidden fixed inset-0 z-50 bg-background flex flex-col">
      {/* Header */}
      <div className="h-14 px-4 border-b border-border/50 flex items-center justify-between bg-background/80 backdrop-blur-md shrink-0">
        ...
        {selectedMemoId ? (
          <div className="p-4 min-h-full flex flex-col">
            <input
              ...
              spellCheck={false}
              autoComplete="off"
              className="w-full px-0 py-2 text-xl font-bold bg-transparent border-none focus:ring-0 placeholder-muted-foreground/50 text-foreground mb-4"
            ...
            <div className="flex-1 relative">
              <textarea
                ...
                spellCheck={false}
                autoComplete="off"
                className="w-full h-full min-h-[50vh] px-0 py-0 bg-transparent border-none focus:ring-0 resize-none font-mono text-sm leading-relaxed text-foreground placeholder-muted-foreground/50"
  ...
  const DesktopSidePanel = () => (
    <div
      className={`hidden md:flex ${open ? (isCopilotOpen ? 'w-96' : 'flex-1') : 'w-0'} transition-all duration-300 bg-background/50 backdrop-blur-sm border-l border-border/50 shadow-xl overflow-hidden flex-col flex-shrink-0 relative`}
    >
      {/* Header */}
